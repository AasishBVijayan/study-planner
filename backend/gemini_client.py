...
class GeminiClient:
    ...
    def generate_response(self, user_input: str) -> str:
        ...
        To trigger web search, start your messege with one of:
        - "search: <query>"
        - "/search <query>"
        Otherwise, the model rseponds directly using chat history
        """
        if not self.chat:
        ...
        try:
            ...
            if search_query:
                ...
                for idx,item in enumerate(web_results, start=1):
                    ref_lines.append(f"[{idx}] {item['title']} - {item['href']}\n{item['body']}")
                refs_block = "\n\n".join(ref_lines)

                system_prompt = (
                    "You are an AI research assisstant. Use the provided web search results to answer the user query"
